...
router.patch('/:id/weeks', validate(schema.idParam, 'params'), validate(schema.updateWeeks), async (req, res, next) => {
  try {
...
router.patch('/:id/course-codes', validate(schema.idParam, 'params'), validate(schema.updateCourseCodes), async (req, res, next) => {
  try {
