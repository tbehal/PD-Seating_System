...
import type { GridResult, GridRow, WeekDate } from '../types';
...
async function buildGrid(
  ...
  const grid: GridRow[] = stations.map((station) => {
  ...
  const weekDates: WeekDate[] = weeks.map((w) => {
...
async function exportGrid(
  ...
  const fmtShort = (d: Date | string | null): string => {
    if (!d) return '';
